// tests/binary_search_tree_preorder_iterative_test.c
#include "unity.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void assert_preorder_equals(const int *expected, int n_expected) {
    const char *CAP = "capture_preorder_iter.txt";
...
void test_empty_tree_prints_nothing(void) {
    const int expected[] = {};
    assert_preorder_equals(expected, 0);
}
...
void test_single_node(void) {
    ...
    assert_preorder_equals(expected, 1);
}

// 3) 문제 예시 구조: {20, 15, 10, 18, 50, 25, 80}
// Preorder: 20, 15, 10, 18, 50, 25, 80
void test_prompt_example_preorder(void) {
    int vals[] = {20, 15, 10, 18, 50, 25, 80};
    insert_many(&root, vals, sizeof(vals)/sizeof(vals[0]));
    const int expected[] = {20, 15, 10, 18, 50, 25, 80};
    assert_preorder_equals(expected, sizeof(expected)/sizeof(expected[0]));
}

// 4) 우측 편향(증가 수열 삽입) → 전위는 삽입 순서와 동일
void test_right_skewed_tree(void) {
    int vals[] = {10, 20, 30, 40, 50};
    insert_many(&root, vals, 5);
    const int expected[] = {10, 20, 30, 40, 50};
    assert_preorder_equals(expected, 5);
}

// 5) 다층 일반 트리
// vals: {50,30,70,20,40,60,80,35,45,65,85}
// Preorder: 50,30,20,40,35,45,70,60,65,80,85
void test_deeper_tree_preorder(void) {
    int vals[] = {50, 30, 70, 20, 40, 60, 80, 35, 45, 65, 85};
    insert_many(&root, vals, sizeof(vals)/sizeof(vals[0]));
    const int expected[] = {50, 30, 20, 40, 35, 45, 70, 60, 65, 80, 85};
    assert_preorder_equals(expected, sizeof(expected)/sizeof(expected[0]));
}
...
int main(void) {
    ...
    RUN_TEST(test_prompt_example_preorder);
    RUN_TEST(test_right_skewed_tree);
    RUN_TEST(test_deeper_tree_preorder);
    return UNITY_END();
}
